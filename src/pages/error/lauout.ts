...
export function ErrorPage(): HTMLElement {
  ...
  const mainButton = createButton({
    classes: [...BUTTON.general, ...BUTTON.generalHover, ...BUTTON.generalFocus, 'uppercase'],
    events: { click: () => console.log('go to main page') },
    text: 'Main page',
  });

  const infoWrapper = createDiv({
    children: [errorNumber, errorTitle, errorText, mainButton],
